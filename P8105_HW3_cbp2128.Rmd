...
```{r prelim_imports, message = FALSE }
library(tidyverse)
library(knitr)
...
The result is the tibble above. There were `r nrow(states_w_7_locs_in_2002)` states (Conneticut, Florida, and North Carolina) with 7 locations listed in 2002. This result suggests that only a few states had such extensive surveying done in the early years of the dataset. 
...
```{r P1_spaghetti_states, message = FALSE }
...
```{r P1_five_panel_bonanza, warning = FALSE, fig.height = 8 }
...
`aisle_grouping` tells us that there are `r nrow(aisle_grouping)` aisles. By counting the number of times an `aisle_id` appears, we can use this to find out which aisles have most items. Each row represents an order, so we would have the aisles with the most items ordered.
...
```{r P2_items_by_aisles, fig.height = 15 }
...
Instead of having bars, I chose to represent the counts as points to make it easier to see the trend in the graph. With so many aisles, it's easy to overcrowd the plot, bars would exacerbate the crowding. The aisle names also line up with the axes of a point, so it makes it easier to see which points are paired to which aisle.
...
```{r P2_popular_items_by aisle, warning = FALSE }
...
```{r P2_pink_lady_apples_are_terrible }
...
In its raw form, the NOAA data set is quite large, coming in at `r nrow(raw_NY_NOAA)` rows and `r ncol(raw_NY_NOAA)` columns. The key variables in the dataset describe precipitation (`prcp`), snowfall (`snow`) and temperature (`tmax` and `tmin`) from 01-01-1981 to 12-31-2010. Missing values are *rampant* in the dataset, since we are told that some weather stations collect subsets of the variables. Some rows have missing data for all the weather variables, indicating that neither rain nor snow fell, and temperature was not recorded at that station. With such a large extent of the data missing, we must be midndful of how we filter and look at the data. 
...
```{r P3_wheres_my_snow, warning = FALSE, message = FALSE }
...
```{r temp_thru_time, warning = FALSE }
